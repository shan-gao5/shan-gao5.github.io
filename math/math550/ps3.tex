...
\subsection*{(a)}
We want to use fractional helly's theorem with $\alpha = \frac{1}{4}$, want to show that there exists at least $\alpha \binom{n}{3} = \frac{\binom{n}{3}}{4}$ sets $I$ where $|I| = 3$ where $\bigcap_{i \in I} C_i \neq \emptyset$ to be able to conclude that there exists some $X$ such that $|X| \geq n \beta = \frac{n}{12}$ where $\beta = \frac{\alpha}{3}$ such that $\bigcap_{i \in X} C_i \neq \emptyset$. We know that there are $\binom{n}{4}$ ways of forming a bundle of 4 sets, each of which contains a triplet that share a common element, by assumption. But each triplet is contained in $n-3$ bundles of 4 sets. That means, there are $\displaystyle \frac{\binom{n}{4}}{n-3}$ sets $I$ where $|I| = 3$ such that $\bigcap_{i \in I} C_i \neq \emptyset$. But observe that 

\begin{align*}`	
	\frac{\binom{n}{4}}{n-3} &= \frac{n!}{(n-4)!4!(n-3)}\\
				 &= \frac{n!}{(n-3)!3!} \cdot \frac{1}{4}\\
				 &= \frac{\binom{n}{3}}{4}
\end{align*}

\subsection*{(b)}
We want to use fractional Helly's theorem again. We want to get a lower bound on the number of $I$'s such that $|I| = d+1$ such that $\displaystyle \bigcap_{i \in I} C_i \neq \emptyset$. We know that every bundle of p sets that we choose contains a sub-bundle of $d+1$ sets with a common element, there are $\displaystyle \binom{n}{p}$ ways of choosing such a bundle, but every sub-bundle of $d+1$ sets is contained in $\displaystyle \binom{n-(d+1)}{p-(d+1)}$ bundles of $p$ sets, therefore there exists at least the following amount of $I$'s with the proprety: 
\begin{align*}
	\binom{n}{p} \cdot \frac{1}{\binom{n-(d+1)}{p-(d+1)}} &= \frac{n!}{p! (n-p)!} \cdot \frac{(p-(d+1))! (n-p)!}{(n- (d+1))!}\\
							      &= \frac{n!}{(d+1)! (n-(d+1))!} \cdot \frac{(p-(d+1))!(d+1)!}{p!}\\
							      &= \binom{n}{d+1} \cdot \frac{1}{\binom{p}{d+1}}
\end{align*}
Could've gotten the same result knowing that
$$
	\binom{n}{p}\binom{p}{d+1} = \binom{n}{d+1}\binom{n-(d+1)}{p-(d+1)}
$$
By double counting pairs of (bundle, sub-bundle)
The result follows immediate by fractional Helly's letting $\alpha = \frac{1}{\binom{p}{d+1}}$ and $c = \beta = \frac{\alpha}{d+1}$

\subsection*{(c)}
This is probably done by following the proof for fractional Helly's, let's try it out. 
